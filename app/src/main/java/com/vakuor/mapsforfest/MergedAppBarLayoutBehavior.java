...
public class MergedAppBarLayoutBehavior extends AppBarLayout.ScrollingViewBehavior {
    ...
    public boolean onDependentViewChanged(CoordinatorLayout parent, View child, View dependency) {
        ...
        if(isDependencyYBelowAnchorPoint(parent, dependency)){
            ...
            setFullBackGroundColor(com.mahc.custombottomsheetbehavior.R.color.colorPrimary);
            setPartialBackGroundHeight(0);
        }
        return childMoved;
    }
...
//        setStatusBarBackgroundVisible(mVisible);
        ...
        setFullBackGroundColor(mVisible && mCurrentTitleAlpha == 1 ? com.mahc.custombottomsheetbehavior.R.color.colorPrimary: android.R.color.transparent);
        setPartialBackGroundHeight(0);
    ...
    private TextView findTitleTextView(Toolbar toolbar){
        for (int i = 0; i < toolbar.getChildCount(); i++) {
            ...
            if (toolBarChild instanceof TextView &&
                    ...
                    ((TextView)toolBarChild).getText().toString().contentEquals(mContext.getResources().getString(com.mahc.custombottomsheetbehavior.R.string.key_binding_default_toolbar_name))) {
    ...
    private boolean isStatusBarVisible(){
        if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP){
            return ((Activity)mContext).getWindow().getStatusBarColor() ==
                    ContextCompat.getColor(mContext, com.mahc.custombottomsheetbehavior.R.color.colorPrimaryDark);
        }
        return true;
    }
    ...
    private void setStatusBarBackgroundVisible(boolean visible){
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP){
            if(visible){
                ...
                window.setStatusBarColor(ContextCompat.getColor(mContext, com.mahc.custombottomsheetbehavior.R.color.colorPrimaryDark));
            }else {
                Window window = ((Activity)mContext).getWindow();
